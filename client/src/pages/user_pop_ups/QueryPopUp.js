...
function QueryPopUp({ queryClicked, setQueryClicked, setListOfUserPurchases, setIsQuerying }) {
  ...
  const queryInfo = (timeOption, priceDirOption, flavourOption, data) => {
    ...
    if (timeOption) {
      const { time } = data;
      fetch(`http://localhost:3001/user/orderByTime/${userId}/${time}`, {
        headers: {
      ...
      fetch(`http://localhost:3001/user/orderByPrice/${userId}/${direction}`, {
        headers: {
          accessToken: localStorage.getItem("accessToken"),
        },
      })
      ...
      const { flavour } = data;
      fetch(`http://localhost:3001/user/orderByFlavour/${userId}/${flavour}`, {
        headers: {
          accessToken: localStorage.getItem("accessToken"),
        },
      })
